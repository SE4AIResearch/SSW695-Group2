...
**Implementation status:** Phases 1–6 built and tested. Full MVP P0 worker pipeline complete.
...
`AssigneeSelector.select()` receives an `AsyncSession` from `EventProcessorService` rather than managing its own session. This keeps the `open_assignments` increment and the `TriageDecision` write (Phase 5) in a **single atomic transaction** — both commit together or neither does. `EventProcessorService` calls `session.commit()` after all Phase 4 and 5 writes complete. (See DD-20.)
...
```
Phase 5 complete — TriageDecision written, patch_state = DECIDED
│
├── github_client not configured → log warning, return (patch skipped)
│
▼
Step 6.1 — Build JWT
  Sign payload {iss: app_id, iat: now, exp: now+600} with App private key (RS256)
│
▼
Step 6.2 — Exchange JWT for installation access token
  POST /app/installations/{installation_id}/access_tokens
  ├── Error → FAILED_RETRY, patch_attempts += 1, last_error = message, return
  └── OK   → installation_token
│
▼
Step 6.3 — Apply labels and assignee
  PATCH /repos/{owner}/{repo}/issues/{number}
  Body: { "labels": [<existing> + category + priority (deduped)],
          "assignees": [assignee_login] }  ← key omitted if no assignee
  ├── 429 / 5xx  → FAILED_RETRY, patch_attempts += 1, last_error = message, return
  ├── other 4xx  → FAILED_RETRY + write DLQRecord, return
  └── OK         → continue
│
▼
Step 6.4 — Post explanation comment
  POST /repos/{owner}/{repo}/issues/{number}/comments
  Body: { "body": TriageDecision.explanation }
  ├── 429 / 5xx  → FAILED_RETRY, patch_attempts += 1, last_error = message, return
  ├── other 4xx  → FAILED_RETRY + write DLQRecord, return
  └── OK         → continue
│
▼
Step 6.5 — Mark patch complete
  UPDATE triage_decision SET patch_state = 'APPLIED' WHERE event_id = ?
```

#### Authentication

GitHub App authentication requires two steps (DD-06):
```
Step 1: Sign a short-lived JWT using the App private key (RS256)
...
Step 3: Use the installation token for all API calls
        Authorization: Bearer {token}
```

A fresh token is requested per event — no caching in MVP.

#### Label construction

```python
final_labels = list(event.issue.labels)          # preserve existing labels
for label in [result.category, result.priority]:  # add buma labels
    if label not in final_labels:
        final_labels.append(label)
```

This avoids replacing labels the team applied manually before buma processed the issue.

#### `patch_state` lifecycle

| State | Meaning |
|---|---|
| `DECIDED` | Written by Phase 5; GitHub patch not yet attempted |
| `APPLIED` | All GitHub API calls succeeded; patch complete |
| `FAILED_RETRY` | One or more calls failed; retry pending (manual or future mechanism) |
| `SKIPPED_DUPLICATE` | Reserved for future use |

#### Error classification

| HTTP status | Classification | Action |
|---|---|---|
| `429` or `5xx` | Transient | `FAILED_RETRY`, `patch_attempts += 1`, `last_error` stored |
| Other `4xx` (e.g. `404`, `422`) | Non-retryable | `FAILED_RETRY` + write `DLQRecord` |

#### Implementation

- `GitHubClient` in `src/buma/worker/services/github_client.py` — single-responsibility async client (JWT build, token exchange, issue patch, comment post)
- `EventProcessorService` receives `github_client: GitHubClient | None` via constructor injection; `None` → Phase 6 skipped safely
- `runner.py` instantiates `GitHubClient` from `GITHUB_APP_ID` and `GITHUB_APP_PRIVATE_KEY` settings if both are present

---

## 5. Error Handling and the Dead-Letter Queue

Any unhandled exception that escapes `EventProcessorService.process()` is caught by `QueueConsumer.run_forever()`, which logs it and continues the loop. No `DLQRecord` is written at the consumer level — that is `EventProcessorService`'s responsibility.

Within `EventProcessorService`, exceptions are classified:

| Exception type | Action |
|---|---|
| `IntegrityError` on Phase 5 INSERT (persistence) | Treat as duplicate — log, return, no DLQ |
| Transient DB/network error | Raise (consumer loop catches and continues; message is lost — detectable via `WebhookDelivery` table) |
| Non-retryable GitHub API error | Write `DLQRecord`, update `patch_state`, return |
| Any unexpected exception | Raise (consumer loop catches and logs) |

The `DLQRecord` captures:

| Column | Value |
|---|---|
| `event_id` | `event.event_id` |
| `delivery_id` | `event.delivery_id` |
| `event_payload` | full `NormalizedEvent` as JSONB |
| `error_message` | exception string |
| `error_type` | `"CLASSIFICATION"`, `"ASSIGNMENT"`, or `"GITHUB_PATCH"` |
...
The runner installs handlers for `SIGINT` and `SIGTERM` that set a shared `asyncio.Event`. The consumer loop checks this event **before each call to `run_once()`**. This means:

- Any message currently being processed completes normally.
- No message is abandoned mid-pipeline.
- After the current `run_once()` returns, the loop exits.
- The runner's `finally` block closes Redis and disposes the SQLAlchemy engine.

This integrates cleanly with Kubernetes pod termination: `SIGTERM` → stop event set → current message drains → process exits → Kubernetes replaces the pod.

(See DD-16 for the full rationale and alternatives considered.)
...
NormalizedEvent
│
│  event_id, delivery_id          ──► IssueSnapshot.event_id (UNIQUE)
│                                  ──► TriageDecision.event_id (UNIQUE)
│  repo.id                         ──► RepoConfig lookup (Phase 2)
│                                  ──► DeveloperProfile query (Phase 4)
│  issue.title, issue.body,        ──► Category classification (Phase 3a)
│  issue.labels, RepoConfig        ──► Priority classification (Phase 3b)
│                                  ──► Assignee skills matching (Phase 4)
│  issue.*                         ──► IssueSnapshot columns (Phase 5)
│  installation_id                 ──► GitHub App token exchange (Phase 6)
│  repo.full_name, issue.number    ──► GitHub API calls (Phase 6)
│  sender_login, trace_id          ──► Logging/tracing only
```
...
The Worker is designed to run as a single async process. The event loop handles the BRPOP timeout and DB/GitHub I/O concurrently within a single thread.

For horizontal scaling (multiple Worker pods):
- The Redis List `BRPOP` distributes messages across all competing consumers automatically — each message is delivered to exactly one worker.
- The `DeveloperProfile.version` optimistic lock handles concurrent assignment attempts across pods without a distributed lock.
- The `UNIQUE` constraints on `IssueSnapshot.event_id` and `TriageDecision.event_id` prevent duplicate persistence in the rare case of replayed messages.

No additional concurrency control is needed at the MVP scale.
...
|---|---|---|
| `Design-Decisions.md` | `Architecture/` | DD-14–19: worker infrastructure and triage scope decisions; DD-05–07: data contract decisions used by this pipeline |
| `Architecture-MVP-Scoped.with-diagrams.md` | `Architecture/` | §4.4.2: Triage Worker component overview; §4.5: data contract pipeline; §4.6: error handling |
| `Architecture-Review-Gaps.md` | `Architecture/` | GAP-01 (idempotency), GAP-02 (optimistic locking), GAP-07 (rules-first, ML deferred) |
| `src/buma/schemas/normalized_event.py` | Buma repo | Canonical `NormalizedEvent` definition — all field names and types used in this document |
| `src/buma/db/models.py` | Buma repo | `IssueSnapshot`, `TriageDecision`, `DeveloperProfile`, `DLQRecord` ORM models |
